...
- [ ] **(사용자)** 폰에서 재확인 — 검색·즐겨찾기·직접등록·삭제·AI 재호출 안 되는지
- **완료 기준:** 식약처에 매칭되는 흔한 음식도, AI로 추정한 음식도 똑같이 즐겨찾기/자주먹는 목록에서 한 탭으로 다시 기록할 수 있고, 한 번 저장된 AI 추정 음식은 같은 이름으로 또 AI를 부르지 않는다.

## Phase 7+ · 계속 붙일 것 (백로그)
- 일일 목표(kcal/매크로) + 홈 목표 대비 표시 — BMR·TDEE 등 계산값 세트(`03` "계산값" 참고). D15(체중목표 우선)와 우선순위 재검토. **D15가 "Phase 3 이후 재검토"라 명시한 조건은 충족됨(Phase 3 완료·폰 확인 끝).**
