...
- [x] 단일 사용자 잠금: `proxy.ts`(D13 — Next 16부터 `middleware.ts`의 새 이름) + `/unlock` + `APP_PASSCODE`
- [ ] drizzle + Neon 실제 연결, `.env.local`에 `DATABASE_URL` 채우기 + 첫 마이그레이션 (Neon 프로젝트 생성 후)
- [ ] Vercel 배포 파이프라인 확인 (GitHub 원격 연동 후)
- **완료 기준:** 빈 앱이 Vercel에서 뜨고, 잠금 통과되고, DB 연결됨.
