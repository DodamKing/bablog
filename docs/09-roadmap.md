...
- [~] **(사용자)** Vercel 프로젝트 env에 VAPID/CRON_SECRET 등록 + 배포 후 폰에서 구독·수신·스킵 확인 — **배포본에서 권한 요청 배너 노출까지 확인(2026-06-25). 실제 구독 완료/수신은 아직 미확인.**
...
- [x] **끼니 태그 모아보기 + 상세보기(통합).** 히스토리 일별 화면을 끼니(아침/점심/저녁/간식)별로 그룹핑 — 같은 끼니를 여러 번에 나눠 기록해도(예: 점심을 두 번에 나눠 입력) 그 끼니 그룹은 합쳐서 하나로 보여줌(그룹 합계 + 그룹에 속한 모든 기록의 음식 항목을 합쳐 이름/양/단위/매크로별로 나열). 수정·삭제는 그룹 안의 개별 기록(레코드) 단위로 유지. 나트륨/당류 등 확장 영양정보는 범위 밖(Phase 6+ "음식별 영양정보 확장")
...
- **지난 날짜 기록 입력.** 현재 기록 홈에서 저장하면 `eatenAt`이 항상 지금(now)으로 들어감 — 사진/검색 분석 후 보정 화면에 "먹은 날짜·시간" 선택 UI가 없어, 어제/그제 깜빡한 끼니를 나중에 기록할 방법이 없음(2026-06-25 사용자 지적). `app/api/meals` POST는 이미 `eatenAt`을 받게 되어 있음(`data.eatenAt` 있으면 사용) — 빠진 건 UI(날짜·시간 선택기)뿐. `MealEditor`에 옵션으로 추가하면 신규 기록·수정 양쪽에 동시 적용 가능.
