...
## 현재 상태
- **단계:** Phase 1 배포·동작 완료. 입력 개선(D18) 진행 중 — 보정 화면까지 끝, **수동 입력(식약처 DB 검색)만 남음**.
- **다음 작업(다음 세션):** 식약처 API로 **검색 매칭 동작 + 응답 필드명 확인**(아래 D18 구현 메모) → 검색 route + 검색 UI + AI 폴백 연동 → 그 후 Phase 1.5(PWA·브랜딩).
- **구현 코드:** Next.js 16.2.9. 카카오 로그인(D16)·기록 루프(사진→분석→보정→저장)·R2 사진 보관·**Vercel 배포(bablog.dimad.kr) 동작 확인**. UI 리디자인(D14 톤, Jua+크림+코랄+🍚). 사진 리사이즈/EXIF 제거. dev 로그인 우회(`DEV_AUTH_BYPASS`). 보정 화면 D18 리팩터(끼니 선택 + 항목별 양/단위 스테퍼 + 삭제 되돌리기 + 비음식 가드). 식약처 키 발급·활성화 확인(`FOOD_API_KEY`).

## Phase 체크
- [x] Phase 0 · 프로젝트 셋업 (Neon·Vercel·카카오 로그인 완료)
- [~] Phase 1 · 기록 루프 (사진 기록·배포 완료, 수동 입력만 남음)
- [ ] Phase 1.5 · PWA화
- [ ] Phase 2 · 조회 (히스토리 + 체중)
- [ ] Phase 3 · 보고서 + AI 조언
- [ ] Phase 4 · 푸시 알림
