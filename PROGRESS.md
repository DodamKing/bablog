...
## 현재 상태
- **단계:** **Phase 0·1 완료.** 카카오 로그인·Vercel 배포, 사진 기록 + 수동 입력(식약처 검색 + AI 폴백), R2 사진 보관, 브라우저 E2E(저장→DB) 모두 동작 확인.
- **다음 작업:** **Phase 1.5(PWA·브랜딩)** — 로고/아이콘(파비콘·192/512/maskable·OG), manifest, 서비스 워커(셸 캐싱), 안드로이드 홈 설치 standalone 확인. 읽을 문서: `07`.
